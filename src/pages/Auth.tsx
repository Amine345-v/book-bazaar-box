        ...
        <div className="bg-card rounded-xl p-8 shadow-book border border-border">
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="bg-card px-2 text-muted-foreground font-body">{t("auth.orContinueEmail")}</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {!isLogin && (
              <div>
                <Label htmlFor="name" className="font-body text-sm">{t("auth.displayName")}</Label>
              ...
              {!isLogin && password.length > 0 && (
                <div className="mt-2 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`font-body text-xs font-medium ${strength >= 3 ? "text-emerald-600" : "text-muted-foreground"}`}>
                      {strengthLabels[strength - 1] || "Very Weak"}
                    </span>
                  </div>
                  <ul className="space-y-0.5">
                    {[
                      { test: password.length >= 8, label: t("auth.passwordRules.length") },
                      { test: /[A-Z]/.test(password), label: t("auth.passwordRules.uppercase") },
                      { test: /[0-9]/.test(password), label: t("auth.passwordRules.number") },
                      { test: /[^A-Za-z0-9]/.test(password), label: t("auth.passwordRules.special") },
                    ].map((rule) => (
                      <li key={rule.label} className="flex items-center gap-1.5 font-body text-xs text-muted-foreground">
                        {rule.test ? (
            ...
            <Button type="submit" className="w-full font-body font-semibold" disabled={loading}>
              {loading ? t("auth.loading") : isLogin ? t("auth.signInBtn") : t("auth.createAccountBtn")}
            </Button>
          </form>
