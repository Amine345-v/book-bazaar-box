      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {testimonials.map((item, i) => (
            <div key={i} className="bg-card rounded-xl p-6 shadow-book relative">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="font-body font-semibold text-card-foreground text-sm">{item.name}</p>
                  <div className="flex gap-0.5">
                    {[...Array(5)].map((_, j) => (
                      <Star
                        key={j}
                        className={`h-3 w-3 ${j < item.rating ? "fill-primary text-primary" : "text-border"}`}
                      />
                    ))}
                  </div>
                </div>
